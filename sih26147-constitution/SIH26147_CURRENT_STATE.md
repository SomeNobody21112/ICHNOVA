...
**Date:** 2026-09-17 · **Version:** 2.3 (real transmissions + live monitor + vectorised search; evidence in `reports/REAL_SIGNAL_VALIDATION.md`, `reports/PERFORMANCE_REPORT.md`; previous: 2.2 baseline hardening)
...
| 24 | FSK demodulation (blind tone pair, shift, baud, start-stop framing, ITA2/ASCII) | **PROVEN on real signal** | DWD DDH47: 50 Bd, 85.0 Hz, 825/828 stop bits, p=10^-241, text names its callsign/frequency; synthetic tests | Sync FSK (SITOR-B/NAVTEX) |
...
| 34 | Real public IQ validation | **PROVEN (6 government transmitters)** | WWV, DCF77, MSF, JJY decoded blind, arrival vs GPS +1.9…+23 ms; WWVB time refused; AIR MW 5/5 vs official list; recordings committed, CI regression | Real PSK/FEC modems on air |
| 34a | Live reception (public KiwiSDR receivers, SSE) | **PROVEN** | Live DWD and WWV sessions; replays generated by the same processor | Receiver delay calibration |
| 34b | Vectorised hypothesis search | **PROVEN** | 3.0–3.8× faster, 0 decision differences on sealed/train/null set | — |
...
| 39 | Real-time SDR capture (local hardware) | **SUPERSEDED** | Public KiwiSDR receivers used instead (listen-only, public services) | — |
...
python -m pytest -q tests                # must be 30/30 (core + real-signal recordings)
python sealed_test.py                    # must be 30/30, 0 false accepts (~1.5 s)
python sealed_test.py data/train 100     # currently 63/100, 0 false accepts (~4.5 s)
```
